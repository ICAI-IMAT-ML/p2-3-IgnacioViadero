...
class LinearRegressor:
    ...
    def fit_simple(self, X, y):
        ...
        X_extended = np.c_[np.ones_like(X), X]
        
        
        # TODO: Train linear regression model with only one coefficient

        coefficients = np.linalg.lstsq(X_extended, y, rcond=None)[0]
    ...
    def fit_multiple(self, X, y):
        ...
        X_traspuesta = X.T
        coefficients = np.linalg.inv(X_traspuesta.dot(X)).dot(X_traspuesta).dot(y)
        self.intercept = coefficients[0]
        self.coefficients = coefficients[1:]
